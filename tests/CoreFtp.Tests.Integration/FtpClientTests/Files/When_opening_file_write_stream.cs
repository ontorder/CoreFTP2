...
public class When_opening_file_write_stream : TestBase
{
    ...
    public static async Task<Stream> OpenWriteAsync(FtpClient ftpClient, string path, string fileName)
    {
        await ftpClient.CreateDirectoryAsync(path, default);
        await ftpClient.ChangeWorkingDirectoryAsync(path, default);
        return await ftpClient.OpenFileWriteStreamAsync(fileName, default);
    }
}